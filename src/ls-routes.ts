...
function flattenArray<T, V>(array: T[] | T): V[] {
  return !Array.isArray(array)
    ? array
    : [].concat.apply([], array.map(r => flattenArray(r)));
}
  ...
  return routes.map(route => {
    if (!route.children) {
      ...
      return flattenArray(flattenRouteToPath(route.children)).map(
        (childRoute: string) =>
          (!route.path ? '' : '/' + route.path) + childRoute
      );
    }
  });
}

function coerceIntoPromise<T>(
  mightBePromise: Observable<T> | Promise<T> | T
): Promise<T> {
...
function extractRoute(route: Route, injector: Injector): Promise<Route> {
  ...
  if (route.children) {
    return Promise.all(
      route.children.map(r => extractRoute(r, injector))
    ).then(routes => {
