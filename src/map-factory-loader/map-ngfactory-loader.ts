 ...
 */
export const MODULE_MAP: InjectionToken<IModuleMap> = new InjectionToken(
  'MODULE_MAP'
);
...
export class ModuleMapNgFactoryLoader implements NgModuleFactoryLoader {
  constructor(
    ...
    @Inject(MODULE_MAP) private moduleMap: IModuleMap
  ) {}
  ...
  public load(loadChildrenString: string): Promise<NgModuleFactory<any>> {
    ...
    return offlineMode
      ? this.loadFactory(type as NgModuleFactory<any>)
      : this.loadAndCompile(type as Type<any>);
  }
